	...
	ATTRIBUTE_UNUSED void *cookie)
{
  ...
  if ((void *)(x) == this_obj)
    op (&((*x).actor_decl), NULL, cookie);
  if ((void *)(x) == this_obj)
    op (&((*x).destroy_decl), NULL, cookie);
  if ((void *)(x) == this_obj)
    op (&((*x).promise_type), NULL, cookie);
  if ((void *)(x) == this_obj)
    op (&((*x).handle_type), NULL, cookie);
  if ((void *)(x) == this_obj)
    op (&((*x).self_h_proxy), NULL, cookie);
  if ((void *)(x) == this_obj)
    op (&((*x).promise_proxy), NULL, cookie);
  if ((void *)(x) == this_obj)
    op (&((*x).return_void), NULL, cookie);
}
...
EXPORTED_CONST struct ggc_root_tab gt_ggc_r_gt_cp_coroutines_h[] = {
  ...
  },
  {
    ...
    1,
    sizeof (coro_frame_needs_free_id),
    &gt_ggc_mx_tree_node,
    &gt_pch_nx_tree_node
  },
  {
