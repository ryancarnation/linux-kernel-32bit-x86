/* Type information for objc/objc-next-runtime-abi-02.cc.
...
gt_pch_nx (struct ident_data_tuple* x ATTRIBUTE_UNUSED,
	...
	ATTRIBUTE_UNUSED void *cookie)
{
    op (&((*x).ident), NULL, cookie);
    op (&((*x).data), NULL, cookie);
}
	...
	ATTRIBUTE_UNUSED void *cookie)
{
    ...
    op (&((*x).selname), NULL, cookie);
    op (&((*x).refdecl), NULL, cookie);
}
  ...
  if (extern_names != NULL) {
    ...
    for (i0 = 0; i0 != (size_t)(SIZEHASHTABLE) && ((void *)extern_names == this_obj); i0++) {
      if ((void *)(extern_names) == this_obj)
        op (&(extern_names[i0]), NULL, cookie);
    }
    if ((void *)(&extern_names) == this_obj)
      op (&(extern_names), NULL, cookie);
  }
}
