/* Type information for cp/semantics.cc.
...
void
gt_ggc_mx_hash_map_tree_bool_ (void *x_p)
{
  hash_map<tree,bool> * const x = (hash_map<tree,bool> *)x_p;
  if (ggc_test_and_set_mark (x))
    {
      gt_ggc_mx (x);
    }
...
void
gt_pch_nx_hash_map_tree_bool_ (void *x_p)
{
  hash_map<tree,bool> * const x = (hash_map<tree,bool> *)x_p;
  if (gt_pch_note_object (x, x, gt_pch_p_19hash_map_tree_bool_))
    {
      gt_pch_nx (x);
    }
}
...
gt_pch_nx (struct deferred_access* x ATTRIBUTE_UNUSED,
	...
	ATTRIBUTE_UNUSED void *cookie)
{
    op (&((*x).deferred_access_checks), NULL, cookie);
}

void
gt_pch_p_19hash_map_tree_bool_ (ATTRIBUTE_UNUSED void *this_obj,
	void *x_p,
	ATTRIBUTE_UNUSED gt_pointer_operator op,
	ATTRIBUTE_UNUSED void *cookie)
{
  struct hash_map<tree,bool> * x ATTRIBUTE_UNUSED = (struct hash_map<tree,bool> *)x_p;
  if ((void *)(x) == this_obj)
    gt_pch_nx (&((*x)), op, cookie);
}
...
EXPORTED_CONST struct ggc_root_tab gt_ggc_rd_gt_cp_semantics_h[] = {
  { &boolish_class_type_p_cache, 1, sizeof (boolish_class_type_p_cache), NULL, NULL },
  LAST_GGC_ROOT_TAB
};
