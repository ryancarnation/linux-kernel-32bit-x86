/* Type information for m2/gm2-lang.cc.
...
gt_ggc_mx_language_function (void *x_p)
{
  ...
  if (ggc_test_and_set_mark (x))
    {
      gt_ggc_m_9tree_node ((*x).stmt_tree);
    }
}
...
gt_pch_nx_language_function (void *x_p)
{
  ...
  if (gt_pch_note_object (x, x, gt_pch_p_17language_function))
    {
      gt_pch_n_9tree_node ((*x).stmt_tree);
    }
}
	...
	ATTRIBUTE_UNUSED void *cookie)
{
  ...
  if ((void *)(x) == this_obj)
    op (&((*x).stmt_tree), NULL, cookie);
}

/* GC roots.  */

EXPORTED_CONST struct ggc_root_tab gt_ggc_r_gt_m2_gm2_lang_h[] = {
  {
    &gm2_eh_personality_decl,
    1,
    sizeof (gm2_eh_personality_decl),
    &gt_ggc_mx_tree_node,
    &gt_pch_nx_tree_node
  },
  LAST_GGC_ROOT_TAB
